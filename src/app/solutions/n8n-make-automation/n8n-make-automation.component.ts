...
@Component({
  selector: 'app-n8n-make-automation',
  standalone: true,
  imports: [
    RouterModule,
  ...
  templateUrl: './n8n-make-automation.component.html',
  styleUrl: './n8n-make-automation.component.scss',
})
export class N8nMakeAutomationComponent implements AfterViewInit { 
  constructor(
    private meta: Meta,
    ...
    this.meta.addTag({ name: 'title', content: 'N8n and Make.com Automation - Distinct Cloud Labs' });
  }
  ...
  ngOnInit(): void {
    this.meta.updateTag({
      name: 'title',
      content: 'N8n and Make.com Automation - Distinct Cloud Labs',
    });
    this.meta.updateTag({
      name: 'description',
      content: 'Explore successful AI implementations.',
    });
  ...
  initializeMarquee(): void {
    if (this.common.isBrowser() && typeof jQuery !== 'undefined' && jQuery.fn.marquee) {
      const marqueeElement = jQuery('app-n8n-make-automation .marquee_text');
